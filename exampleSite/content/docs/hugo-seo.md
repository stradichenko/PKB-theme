...
#### Native sitemap (Hugo built-in)
The sitemap is generated by Hugo itself from the `[sitemap]` config table; no Node.js post-processing step is needed. (The legacy `scripts/generate-sitemap.js` helper was removed in 2026-07: it ran outside the build and overwrote Hugo's own output.)
...
Hugo Build
    ...
    └─ Sitemap Generation (native Hugo [sitemap] output)
    ↓
