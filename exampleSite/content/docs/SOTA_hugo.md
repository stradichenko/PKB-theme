...
|---|---|---|---|
| Version floor | `theme.toml` min 0.136.0; README (working tree) claims "v0.148+"; CI pins 0.123.7 ([test.yml](.github/workflows/test.yml)) and 0.128.0 ([hugo.yml](.github/workflows/hugo.yml)) | v0.164.0; `useEmbedded` needs ≥0.148.0 (F6) | **CI pins are below the theme's own min_version** - bump CI to ≥0.148.0 (or latest); align `theme.toml` min_version with the README's 0.148+ claim |
| CI health | [test.yml](.github/workflows/test.yml) contains an **unresolved git merge conflict** (`<<<<<<< HEAD` / `>>>>>>> gh-pages`) | - | Resolve before any CI run is meaningful |
| Render-hook config | Working tree: `useEmbedded = 'fallback'` in [hugo.toml](config/_default/hugo.toml) + new [markup.toml](config/_default/markup.toml) with footnote/passthrough | Exactly the v0.148.0 enum form (F6); passthrough config matches docs (F5) | Correct - complete and commit the migration |
...
| Scratch | `.Page.Scratch.Get/Set` in [cite.html](layouts/shortcodes/cite.html) (4 calls) and [sidenote.html](layouts/shortcodes/sidenote.html) (2 calls) | `.Page.Store`; Scratch **deprecated v0.138.0** (F7) | Mechanical rename to `.Page.Store` |
| Imaging config | Global `quality = 85` under `[imaging]` ([hugo.toml:73](config/_default/hugo.toml)) | Global `imaging.quality` deprecated v0.163.0 (F23) | Move to `[imaging.webp]` / `[imaging.avif]` per-format blocks |
| Language config | `languageCode = 'en-us'` ([hugo.toml:2](config/_default/hugo.toml)) | `languageCode` → `locale` deprecated v0.158.0 (F12) | Rename key |
...
| QR codes | [qr.html](layouts/shortcodes/qr.html) calls external `api.qrserver.com`; [page-qr.html](layouts/shortcodes/page-qr.html) uses native `images.QR` | `images.QR` native since v0.141.0 (F8) | Migrate `qr.html` to `images.QR`; drop the external dependency |
| Math rendering | KaTeX loaded client-side ([head.html](layouts/partials/head.html)) **and** MathJax via [math.html](layouts/partials/math.html) - two renderers referenced | Server-side pipeline: passthrough + `transform.ToMath` (F25) | Pick one renderer; consider server-side KaTeX to match the new passthrough hook |
| RSS link | Hard-coded `{{ .Site.BaseURL }}index.xml` in [footer.html](layouts/partials/footer.html) | `.OutputFormats.Get "RSS"` | Minor robustness fix |
...
| Menu entries | `.URL` on menu entries in [menu.html:23](layouts/partials/menu.html) | `.PageRef` is the documented modern accessor | [UNVERIFIED: formal deprecation status] - low-priority cleanup |
